...
def getProductos():
    ...
    print("Productos traídos")
    return jsonify(user), 200

import logging

# Configurar el registro de eventos de Flask
app.logger.setLevel(logging.ERROR)  # Establecer el nivel de registro a ERROR o superior
...
def getCarritos():
    try:
        ...
        if not carritos:
            error_message = {'error': 'No se encontraron carritos'}
            return jsonify(error_message), 404

        carritos_serialized = [carrito.serialize() for carrito in carritos]
        return jsonify(carritos_serialized), 200
    except Exception as e:
        error_message = {'error': 'Error del sistema'}
        return jsonify(error_message), 500
...
def addCarrito():
    try:
        ...
        existing_carrito = Carrito.query.filter_by(id_carrito=id_carrito).first()
        if existing_carrito:
            error_message = {'error': 'El carrito ya existe'}
            return jsonify(error_message), 409

        # Crear el objeto Carrito
        carrito = Carrito(id_carrito=id_carrito, rut=rut, total=total)
        ...
        error_message = {'error': 'Error al agregar el carrito'}
        return jsonify(error_message), 500
...
def addProductoCarrito():
    try:
        ...
        if not id_producto or not id_carrito:
            return jsonify({'mensaje': 'Error de sintaxis. Campos requeridos no proporcionados'}), 400
